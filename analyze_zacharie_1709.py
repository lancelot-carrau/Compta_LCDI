...
# -*- coding: utf-8 -*-
"""
Analyse spécifique de la facture Zacharie Carpentier (1709)
"""
import sys
import os
...
def analyze_zacharie_1709():
    """Analyser la facture Zacharie Carpentier (1709)"""
    ...
    print(f"📄 Analyse de la facture Zacharie Carpentier: {pdf_file}")
    print("=" * 80)
    ...
    try:
        ...
        # Chercher spécifiquement les montants attendus pour Zacharie
        print("\n💰 RECHERCHE DES TOTAUX ZACHARIE (193,32€ HT / 38,66€ TVA / 231,98€ Total):")
        print("-" * 50)
        for i, line in enumerate(lines):
            line_clean = line.strip()
            if ('193' in line_clean and ('32' in line_clean or '66' in line_clean)) or \
               ('38' in line_clean and '66' in line_clean) or \
               ('231' in line_clean and '98' in line_clean) or \
               ('zacharie' in line_clean.lower() or 'carpentier' in line_clean.lower()):
                print(f"Ligne {i+1}: {line_clean}")
